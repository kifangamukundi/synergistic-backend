...
export const isModerator = (req, res, next) => {
  if (req.user && req.user.isModerator) {
    ...
    res.status(401).send({ message: 'Invalid Moderator Token' });
  }
};
export const isAdminOrModerator = (req, res, next) => {
  if (req.user && (req.user.isAdmin || req.user.isModerator)) {
    ...
    res.status(401).send({ message: 'Invalid AdminOrModerator Token' });
  }
};
export const isFieldAgent = (req, res, next) => {
  if (req.user && req.user.isFieldAgent) {
    next();
  } else {
    res.status(401).send({ message: 'Invalid Field Agent Token' });
  }
};
